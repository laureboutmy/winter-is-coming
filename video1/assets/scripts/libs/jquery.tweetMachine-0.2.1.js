	...
	$.fn.tweetMachine = function (query, options, callback) {
		...
		$(this).each(function () {
			...
			if (this.tweetMachine) {
				...
				settings = $.extend({
					backendScript:  'assets/twitter/getFromTwitter.php', // Path to your backend script that holds your Twitter credentials and calls the API
					...
					tweetFormat: "<li class='tweet'><div class='meta'><img class='avatar' src=''/><a href='' class='username'></a><a href='' class='time'></a><div class='clear'></div></div><p class='tweet-content'></p><div id='actions'><button class='reply'>REP</button><button class='retweet'>RT</button><button class='favorite'>FAV</button></div></li>", // Format for each tweet
				...
				this.tweetMachine = {
					...
					buildTweet: function (tweet) {
                        ...
                        /* RAJOUT DORIAN */
                        tweetObj.find('.reply')
                            .attr('data-username', tweet.user.screen_name);

                         tweetObj.find('.retweet')
                            .attr('data-tweetid',  tweet.id_str);
